// Runs the default CST and embedded-language evidence tests with an observation file and reports
// how many positive CST and embedded-language cells of the pre-merge plan their records complete.
...
execFileSync('cargo', ['test', '--all-features', '--test', 'unit', 'default_cst_expectations::every_rust_'], { cwd: path.join(root, 'rust'), env, stdio: 'ignore' });
...
const cells = buildEvidencePlan(manifest, 'pre-merge')
  ...
  .filter(({ cell }) => /^i195-(cst|embed)-.*-positive$/u.test(cell.testId));
const incomplete = cells.filter(({ cell }) => !observedEvidenceForCell(cell, records).complete);
console.log(`${records.length} records; ${cells.length - incomplete.length}/${cells.length} positive CST and embedded cells complete`);
