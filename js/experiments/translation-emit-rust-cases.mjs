...
// main effects. Each case is written to <outdir>/<name>/ with its stages
// dumped by translation-stage-dump.mjs, so the Rust port of the emitter can be
// compared with the JavaScript one case by case. The emitter's two unsupported
// errors (floor division on a signed machine integer, printing a structured
// value) are rejected earlier by the frontends or the checker, so crafted IR in
// rust/tests/unit/translation_emit_rust.rs covers them instead.
// Usage: node experiments/translation-emit-rust-cases.mjs <outdir>
