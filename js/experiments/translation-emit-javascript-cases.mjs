...
// and main effects with lets. Each case is written to <outdir>/<name>/ with
// its stages dumped by translation-stage-dump.mjs, so the Rust emitter can be
// compared with the JavaScript one case by case.
// Usage: node experiments/translation-emit-javascript-cases.mjs <outdir>
...
const cases = {
  ...
  'js-checked-cast': ['mjs', `${jsFn('shift', [['n', 'bigint']], 'bigint', '  return n - 5n;')}${jsFn('half', [['n', 'bigint']], 'bigint', "  if (n < 0n) throw new RangeError('half expects a natural number');\n  return n / 2n;")}console.log(half(shift(9n)));\n`],
