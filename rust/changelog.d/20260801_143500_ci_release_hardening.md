...
- Detect code across the complete pull request so a documentation-only final commit cannot suppress Rust validation, keep genuinely empty pushes empty, and fail closed when Git inspection fails.
- Normalize workflow fixtures so parser and multiline policy regression tests remain portable across LF and CRLF checkouts.
