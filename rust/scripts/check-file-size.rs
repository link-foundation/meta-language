...
/// reported (the behaviour used for local runs and pushes to the default
/// branch).
...
/// Warning-band findings are reported only when the file changed, so unchanged
...
fn print_warnings(warnings: &[Finding], changed: Option<&BTreeSet<String>>) {
    let reportable = warnings
        .iter()
        .filter(|finding| should_report(finding, changed))
        .collect::<Vec<_>>();
...
mod tests {
    ...
    fn only_changed_files_in_the_warning_band_are_annotated() {
        ...
        let annotated: Vec<&str> = result
            .warnings
            .iter()
            .filter(|finding| should_report(finding, Some(&changed)))
    ...
    fn missing_changed_file_list_annotates_every_warning() {
        ...
        assert!(should_report(&finding, None));
    }
    ...
    fn empty_changed_file_list_annotates_nothing() {
        ...
        assert!(!should_report(&finding, Some(&changed)));
    }
