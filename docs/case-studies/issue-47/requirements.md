...
| R-19 | Nothing deferred or unimplemented in vision/roadmap | Partial | Issues #37/#39 closed; registries gated by tests (`tests/unit/link_network.rs`) | `docs/parity-roadmap.md` itself defers SQL dialect keys and Delphi-specific coverage |
| R-20 | 100% test coverage copying competitor test cases | Partial | `cargo llvm-cov` job in `.github/workflows/release.yml`; 48 provenance-bearing `PARITY_FIXTURES` | No coverage threshold (`fail_ci_if_error: false`); ported cases are a small fraction of upstream suites |
