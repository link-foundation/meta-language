#!/usr/bin/env bash
#
# create-issues.sh — file the proposed implementation issues for issue #3.
#
# These issues decompose link-foundation/meta-language#3 into actionable work.
...
BLOB_BASE="https://github.com/${REPO}/blob/main/docs/case-studies/issue-3"
...
for f in "$SCRIPT_DIR"/[0-9][0-9]-*.md; do
  ...
  body="${body}"$'\n\n'"---"$'\n'"_Filed from \`docs/case-studies/issue-3/proposed-issues/$(basename "$f")\`. Part of the implementation plan for #3._"
