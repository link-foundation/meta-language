...
The deliverable of this issue is **this case study folder**, not runtime code. It contains: the collected raw data, an online-research fact-check, three library/competitor surveys, a traceable requirements register, a solution plan per requirement, and 16 ready-to-file implementation-issue specifications with an idempotent creation script. The pull request that adds this folder makes **documentation-only** changes; it does not modify `src/` runtime behaviour.
...
1. **Create issues** so the Rust libraries cover in/out processing for the top-10 programming + top-10 natural languages and mixed-mode txt/Markdown/HTML. → 16 specs in [`proposed-issues/`](./proposed-issues/) + a creation script.
2. **Copy all competitor tests** and match every feature similar projects support, so their users can transition. → [`competitor-test-suites.md`](./competitor-test-suites.md) + specs `#15`/`#16`.
...
| Visual Basic | (no official grammar) | Biggest gap — spec `#06` |
| SQL | `tree-sitter-sequel` | Dialect work — spec `#07` |
| Delphi/Object Pascal | generic `tree-sitter-pascal` | Needs Delphi validation — spec `#08` |
...
Online fact-checking ([`online-research.md`](./online-research.md)) confirmed the repo's **TIOBE May 2026** programming ranking and reconciled the **Ethnologue 2025** natural-language ordering (the French/Arabic and Portuguese/Russian boundaries depend on the L1-vs-total-speaker metric — spec `#02` records the chosen convention). It also corrected several round-number estimates carried over from issue #1: the meta-expression lexicon is **351** concepts (not "328"); the per-language binding test count is **~138** (not "90+"); the formal-ai corpus should cite the actual `data/seed/*.lino` + `data/benchmarks/*.lino` files (not the unverified "706"); and the storage crate is **`doublets`** (the repo is `doublets-rs`).

## Recommended solution architecture

[`solution-plans.md`](./solution-plans.md) maps every requirement to a solution and a build-vs-reuse decision, then phases the work into six stages that map one-to-one onto the proposed issues:

- **Phase 0 — close tracking gaps (small, immediate):** add `txt` as a first-class container (`#01`); record the NL ordering convention (`#02`).
- **Phase 1 — the keystone:** tree-sitter adapter + the 7 official grammars (`#03`); mixed-mode embedding via injection (`#04`); NL segmentation + identification (`#05`).
- **Phase 2 — gap grammars (parallelizable):** Visual Basic (`#06`), SQL dialects (`#07`), Delphi (`#08`).
- **Phase 3 — transform & representation:** enrich the substitution/query matcher (`#09`); persistent snapshots (`#10`); unified query+transform surface (`#11`).
- **Phase 4 — semantics (deepest):** self-description roots (`#12`); common concept ontology from meta-expression (`#13`); cross-language reconstruction + formalization config (`#14`).
- **Phase 5 — corpus adoption (broad, ongoing):** external competitor suites (`#15`); ecosystem corpora (`#16`).

## Proposed issues — specs, not auto-filed

Issue #3 asks us to "create issues." Filing 16 GitHub issues is an outward-facing, hard-to-reverse action, so — per this repository's operating guidance — the case study ships the issues as **ready-to-file specifications** in [`proposed-issues/`](./proposed-issues/) plus an idempotent creation script, and leaves the actual filing to a maintainer's single command:

```sh
cd docs/case-studies/issue-3/proposed-issues
./create-issues.sh            # preview (default — creates nothing)
./create-issues.sh --create   # file them (idempotent: skips titles that already exist)
```

The script reads `title`/`labels` from each spec's frontmatter, rewrites relative doc links to absolute GitHub URLs, and only uses labels that already exist in the repo (`enhancement`, `documentation`). The full index, phasing, and dependency table live in [`proposed-issues/README.md`](./proposed-issues/README.md). Cross-references between specs are written as code-styled `#NN` (the spec's file number) rather than live issue links, because the real GitHub issue numbers are unknown until filing — wire up the real "blocked by" relationships afterward using the dependency table.
...
| [`proposed-issues/`](./proposed-issues/) | 16 ready-to-file issue specs + index + `create-issues.sh`. |
